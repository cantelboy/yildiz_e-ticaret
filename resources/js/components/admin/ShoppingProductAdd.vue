    ...
    <form id="SiteAyarForm">
      ...
      <table class="table col-md-12">
        ...
        <tr>
          ...
          <menusTemp @selectMenu="gelenMenu" :menus="CategoryData[0]"
                     :menuSelectData="parseInt(productAdd.productMenuSelect ? productAdd.productMenuSelect : null)"/>
        </tr>
...
export default {
  data() {
    return {
      ...
      menuDatas: [],
      imageUrl:
          "http://127.0.0.1:8000/upload/UrunResimleri/erkek-ayakkabisi/",
      variantData: [],
      variantDbData: []
    };
  },
  ...
  created() {
    ...
    if (!this.$route.params.id == undefined) {
      ...
      .then(response => {
        ...
        console.log("name", this.productAdd.productMenuSelect);
        ...
        this.CategoryData.push(response.data.menu);
        console.log("siteMenu", this.CategoryData);
        this.menuData;
      });
    } else {
  ...
  methods: {
    ...
    SiteProductAdded() {
      console.log('data', this.productAdd);
      var data = {
      ...
      axios.post("/api/admin/products", data).then(response => {
        this.$router.push({ name: 'product.show' });
      });
    },
    ControlFunction(e) {
      e.preventDefault();
    ...
    SiteProductUpdate() {
      ...
      axios.put("/api/admin/products/" + id, data).then(response => {
        console.log(response);
      });
    },
    indexİnc(index) {
  ...
  computed: {
    menuData() {
      setTimeout(() => {
        const total = this.CategoryData[0].reduce((acc, menus) => {
...
>
                                ...
                                <option
                                    v-for="(menu, index) in CategoryData[0]"
                                    :value="menu.id"
                                    :key="index"
                                    :disabled="menu.parentId === null"
                                >
                                    <h6 v-if="menu.parentId === null">**********{{menu.name}}**********</h6>
                                    <h6 v-else>{{menu.name}}+ {{menu.id}}</h6>
